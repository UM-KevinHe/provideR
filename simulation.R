ls <- list(m=1000, gamma1=-5.132825)
m <- ls$m # number of providers
gamma1 <- ls$gamma1 # provider effect for provider 1
rho <- 0
...
do.sim <- function(i) {
  ...
  return(unlist(lapply(1, function(x) 
    bin_logit_cr_score(fail, Z, prov.size, n_prov_pat, 
                       time, cr.logit$gamma, cr.logit$eta, 
                       cr.logit$beta, x, alpha)[21:24])))
}
cl <- parallel::makeCluster(min(n.rep, parallel::detectCores(), 120))
doParallel::registerDoParallel(cl)
`%dopar%` <- foreach::`%dopar%`
system.time(
  res <- foreach::foreach(i=1:n.rep, .combine=cbind,
                          .packages=c("Rcpp", "RcppArmadillo", "MASS"),
                          .errorhandling="remove",
                          .noexport=c("bin_logit_cr", "SRR_bin_logit_cr", 
                                      "bin_logit_cr_score")) %dopar% {
                                        do.sim(i)})
parallel::stopCluster(cl)
matrix(rowMeans(res[,c(0001:1000)+i]!=0), ncol=4, byrow=T)